...
export const SKILLS = ['dsh-dialogue', 'dsh-context-onboarding', 'dsh-understand'];
export const TOOLS = [
...
export function pointerBlock({skillsDir, workspace, label}) {
  return [BEGIN,
    '## 学习系统（DSH-Study）',
    `这台机器装了学习系统，你（${label}）是它的入口，也就是主 agent；执行端是本机的 DSH。用户谈学习、课程、资料，或要你理解他的时候：先读 \`${join(skillsDir, 'dsh-dialogue', 'SKILL.md')}\`（每条消息怎么走、怎么派工给 DSH、怎么验收）和 \`${join(skillsDir, 'dsh-understand', 'SKILL.md')}\`（第二步：理解人）。工作区在 \`${workspace}\`，规则在它的 \`FRAMEWORK.md\` 和 \`protocols/\`。`,
...
/** 把安装包里的三个 skill 装到 tools 里每一个的 skillsDir；dsh-dialogue 附带 connection.local.json、app/、connection-plugin/。返回每个工具装到哪。 */
