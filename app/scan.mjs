// 全扫脚本（protocols/DISCOVERY.md 的程序实现）。由入口 Agent 在 first_run 派工前运行，不用模型；DSH 不扫电脑，只读输出。
...
try {
  ...
  if (existsSync(reg)) { const j = JSON.parse(readFileSync(reg, 'utf8')); vaults = Object.values(j.vaults || {}).map(v => v.path).filter(Boolean); for (const v of vaults) addRoot(v, '笔记库'); }
} catch {}
// 用户目录下深度 ≤ 3 的代码仓（含 .git）
const repos = [];
...
function walk(dir, root, depth) {
  ...
  let ents; try { ents = readdirSync(dir, {withFileTypes: true}); } catch { uncovered.push(dir + '（读不了）'); return; }
  dirs++;
  for (const e of ents) {
    if (stopped) return;
    const p = join(dir, e.name);
...
const coverage = stopped || uncovered.length ? 'partial' : 'partial'; // 全盘从不等于全部；用户在谈话里确认后才升 user_confirmed_enough
const summary = [];
summary.push(`# 全扫 · ${dateTag}`, '', `由入口 Agent 的脚本 \`scan.mjs\` 生成，${isoLocal(now)}（本机 ${hostname()}，${platform()} ${release()}）。只出清单，不复制原文；禁区只登记。DSH 读这份文件摆坐标，从"候选来源"里按需读原文；**DSH 自己不扫电脑**。完整清单在 \`SCAN-${dateTag}-清单.md\`。`, '');
summary.push('## 汇总', '', '| 项 | 值 |', '|---|---|', `| 扫的根 | ${roots.map(r => `${r.label} \`${r.path}\``).join('；')} |`, `| 用时 | ${(elapsedMs / 1000).toFixed(1)} 秒 |`, `| 目录 / 文件 / 字节 | ${dirs} / ${files} / ${fmtBytes(bytes)} |`, `| 预算 | ${MAX_FILES} 文件、${fmtBytes(MAX_BYTES)}、${MAX_MS / 1000} 秒；${stopped ? '**' + stopped + '**' : '未用完'} |`, `| 未覆盖 | ${uncovered.length} 处 |`, `| 禁区 | ${forbidden.length} 处（只登记，未读）；标记 .dsh-private 的目录 ${privateDirs} 个（不列） |`, `| 私密只列 | ${rows.filter(r => r.handle.startsWith('私密')).length}；第三方只列 ${rows.filter(r => r.handle.startsWith('第三方')).length} |`, `| 覆盖状态 | \`${coverage}\`（用户在谈话里确认够用后改 \`user_confirmed_enough\`） |`, '');
...
for (const p of ruleFiles.slice(0, 60)) summary.push(`| 内容区里的规则文件 | \`${cell(p)}\` | \`user_rule_file\` | AI 记忆日志段落按 assistant_summary |`);
for (const p of [...new Set(courseDirs)].slice(0, 60)) summary.push(`| 本地课件目录 | \`${cell(p)}\` | \`inferred_from_behavior\` | 课程名、周次、清单；谈话里确认后升级 |`);
for (const p of calendars.slice(0, 20)) summary.push(`| 日历 | \`${cell(p)}\` | \`inferred_from_behavior\` | 课表、截止；谈话里确认 |`);
for (const p of exportPacks.slice(0, 20)) summary.push(`| AI 对话导出包 | \`${cell(p)}\` | 私密 | 只列；用户允许后才抽用户自己发的消息 |`);
...
for (const f of forbidden.slice(0, 100)) summary.push(`| \`${cell(f.path)}\` | ${f.kind} |`);
if (!forbidden.length) summary.push('| （无） | |');
summary.push('', '## 未覆盖', '', uncovered.length ? uncovered.slice(0, 60).map(u => `- \`${cell(u)}\``).join('\n') : '（无）', '');
...
process.stdout.write(JSON.stringify({
  ...
  forbidden: forbidden.length, private_dirs: privateDirs, known_entries: known.length, rule_files: ruleFiles.length, course_dirs: [...new Set(courseDirs)].length,
  calendars: calendars.length, export_packs: exportPacks.length, coverage, manifest_updated: manifestUpdated,
