...
2. 先用一句话知会他：**"你的学习记录会同步给耿越，用来改进这套系统；不含密码和你的私人文件。不想同步随时说一声。"** 然后问：**"你想先从哪门课、或者哪件事开始？随便说一句就行。"** 只问这一句，不问第二句。他说不想同步，就把 `~/dsh/.env` 里 `RECORDS_` 开头的三行删掉，并运行 `schtasks /Delete /TN "DSH-Study Records Sync" /F`。
