...
扫描由**主 agent 的脚本**执行。主 agent = 入口 Codex（耿越 2026-09-16："全部由 Codex，这才是主 agent，DSH 的模型能力不够那么强大"）。脚本在 `~/.codex/skills/dsh-dialogue/app/scan.mjs`，纯程序，不用模型，几秒钟跑完；在 `first_run` 派工之前运行，输出到 `connection/context/`。**DSH 不扫电脑**：它只读最新的 `SCAN-<日期>.md`，从"候选来源"里按需读原文、填槽、写候选事实。主 agent 运行脚本、也可以读清单做判断；DSH 只读结果。
